...
local function createModel(opt)
   ...
   local iChannels
   local modelParallel = opt.modelParallel
   ...
   local function basicblock(n, stride, type)
      ...
      for i=1,k do
         ...
         ls[i]:add(Convolution(nInputPlane,n,3,3,stride,stride,1,1))
         ls[i]:add(SBatchNorm(n))
         ls[i]:add(ReLU(true))
         ls[i]:add(Convolution(n,n,3,3,1,1,1,1))
         convs:add(ls[i])
      end

      convs:add(shortcut(nInputPlane, n, stride))

      local gpuid = cutorch.getDevice()

      if modelParallel then 
         block:add(nn.ModelParallelTable(2,k,true,true)
                   :add(convs, {gpuid,gpuid+1}))
                   --:threads(function()
                   --   local cudnn = require 'cudnn'
                   --   cudnn.fastest, cudnn.benchmark = true,true
                   --end)) -- CAddTable is done ModelParallelTable
      else
         block:add(convs)
   ...
   local function bottleneck(n, stride, type)
      ...
      for i=1,k do
         ...
         ls[i]:add(Convolution(nInputPlane,n,1,1,1,1,0,0))
         ls[i]:add(SBatchNorm(n))
         ls[i]:add(ReLU(true))
         ls[i]:add(Convolution(n,n,3,3,stride,stride,1,1))
         ls[i]:add(SBatchNorm(n))
         ls[i]:add(ReLU(true))
         ls[i]:add(Convolution(n,n*4,1,1,1,1,0,0))
         convs:add(ls[i])
      end

      convs:add(shortcut(nInputPlane, n * 4, stride)) 
   ...
   if opt.dataset == 'imagenet' then
      ...
      iChannels = 64
      print(' | ResNet-' .. depth .. ' k ' .. k .. ' ImageNet')
   ...
   elseif opt.dataset == 'cifar10' then
      ...
      local n = (depth - 2) / 6
      iChannels = 16
      print(' | ResNet-' .. depth .. ' k ' .. k ..  ' CIFAR-10')
   ...
   --print(model)
   print('Number of Convolutions',#model:findModules('cudnn.SpatialConvolution'))
   print('Number of Parameters', model:getParameters():size()[1])
